...
training_list = './dataset/High_noise/result0.5/training_list.txt'
...
def ensemble_loss(model, train_ids, clean_ids, noise_ids):
    ...
    hard_ids_em = loss_ensemble_self >= 0.05
...
def mean_teacher_train():
    ...
    model, optimizer, scheduler = create_model(pretrained='./dataset/High_noise/result0.5/clean_selected.pth', dropout=False)
    ema_model, _, _ = create_model(pretrained='./dataset/High_noise/result0.5/clean_selected.pth', dropout=True)
    clean_model, _, _ = create_model(pretrained='./dataset/High_noise/result0.5/clean_selected.pth', dropout=False)
    ...
    self_confidence = 1-loss_ensemble_self
    label_confidence = 1 - loss_ensemble
    dataset = CVC_ClinicDB_trnDataSet(img_dir, lbl_dir, list_path=train_list)
    ...
    for epoch in range(maxepoch):
        ...
        for step_t, (volume_batch, label_batch, gt, name) in pbar:
            ...
            #     ema_model.eval()
            #     ema_output_nonoise = ema_model(volume_batch[clean_batchsize:])
            #     ema_output_nonoise = F.softmax(ema_output_nonoise, dim=1)
            #     ema_model.train()
            # breakpoint()
            # for j in range(9):
            #     i = j+7
            #     plt.subplot(2, 2, 1)
            #     plt.imshow(ema_output_nonoise[j, 1, :, :].detach().cpu()>0.5)
            #     plt.axis('off')
            #     plt.title(('outputs_soft', correct_index[name_ensemble.index(name[j + 7])]))
            #     plt.subplot(2, 2, 2)
            #     plt.imshow(label_batch[i , :, :].detach().cpu())
            #     plt.axis('off')
            #     plt.title('label')
            #     plt.subplot(2, 2, 3)
            #     plt.imshow(uncertainty[j, 0, :, :].cpu())
            #     plt.axis('off')
            ...
            # if epoch > 10:
            #     breakpoint()
            #assert labeled_outputs.shape[0] == len(label_ids)
            ...
            consistency_loss = 200 * consistency_weight * consistency_dist
